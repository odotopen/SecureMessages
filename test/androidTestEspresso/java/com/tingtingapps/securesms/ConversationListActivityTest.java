...
import com.tingtingapps.securesms.ConversationActivity;
import com.tingtingapps.securesms.ConversationListActivity;
import com.tingtingapps.securesms.ConversationListItem;
import com.tingtingapps.securesms.GroupCreateActivity;
import com.tingtingapps.securesms.ImportExportActivity;
import com.tingtingapps.securesms.NewConversationActivity;
import com.tingtingapps.securesms.ViewLocalIdentityActivity;
...
import static com.tingtingapps.securesms.EspressoUtil.addContact;
import static com.tingtingapps.securesms.EspressoUtil.waitOn;
import static com.tingtingapps.securesms.ViewMatchers.withRecyclerItem;
import static com.tingtingapps.securesms.ViewMatchers.withRecyclerItemCount;
...
public class ConversationListActivityTest extends TextSecureEspressoTestCase<ConversationListActivity> {
  ...
  private void checkOptionsMenuItemsDisplayed() throws Exception {
    onView(withContentDescription(getContext().getString(com.tingtingapps.securesms.R.string.conversation_list__menu_search)))
          .check(matches(isDisplayed()));

    openActionBarOverflowOrOptionsMenu(getContext());
    onView(withText(com.tingtingapps.securesms.R.string.text_secure_normal__menu_new_group)).check(matches(isDisplayed()));
    onView(withText(com.tingtingapps.securesms.R.string.text_secure_normal__mark_all_as_read)).check(matches(isDisplayed()));
    onView(withText(com.tingtingapps.securesms.R.string.arrays__import_export)).check(matches(isDisplayed()));
    onView(withText(com.tingtingapps.securesms.R.string.arrays__my_identity_key)).check(matches(isDisplayed()));
    onView(withText(com.tingtingapps.securesms.R.string.text_secure_normal__menu_settings)).check(matches(isDisplayed()));

    if (!TextSecurePreferences.isPasswordDisabled(getContext())) {
      onView(withText(com.tingtingapps.securesms.R.string.text_secure_normal__menu_clear_passphrase))
            .check(matches(isDisplayed()));
    }
  ...
  private boolean checkReminderIsDisplayed() throws Exception {
    ...
    if (ExpiredBuildReminder.isEligible(getContext())) {
      reminderTitleResId = com.tingtingapps.securesms.R.string.reminder_header_expired_build;
      reminderTextResId  = com.tingtingapps.securesms.R.string.reminder_header_expired_build_details;
    } else if (DefaultSmsReminder.isEligible(getContext())) {
      reminderTitleResId = com.tingtingapps.securesms.R.string.reminder_header_sms_default_title;
      reminderTextResId  = com.tingtingapps.securesms.R.string.reminder_header_sms_default_text;
    } else if (SystemSmsImportReminder.isEligible(getContext())) {
      reminderTitleResId = com.tingtingapps.securesms.R.string.reminder_header_sms_import_title;
      reminderTextResId  = com.tingtingapps.securesms.R.string.reminder_header_sms_import_text;
    } else if (PushRegistrationReminder.isEligible(getContext())) {
      reminderTitleResId = com.tingtingapps.securesms.R.string.reminder_header_push_title;
      reminderTextResId  = com.tingtingapps.securesms.R.string.reminder_header_push_text;
    } else {
      reminderVisible = false;
    }

    if (reminderVisible) {
      onView(withId(com.tingtingapps.securesms.R.id.reminder_title)).check(matches(isDisplayed()));
      onView(withId(com.tingtingapps.securesms.R.id.reminder_title)).check(matches(withText(reminderTitleResId)));
      onView(withId(com.tingtingapps.securesms.R.id.reminder_text)).check(matches(isDisplayed()));
      onView(withId(com.tingtingapps.securesms.R.id.reminder_text)).check(matches(withText(reminderTextResId)));
    }
  ...
  public static Matcher<Object> isThreadFrom(final String contactName) {
    return new TypeSafeMatcher<Object>() {
      ...
      public boolean matchesSafely(Object object) {
        ...
        TextView             fromView = (TextView) itemView.findViewById(com.tingtingapps.securesms.R.id.from);
  ...
  public static Matcher<Object> withThreadSnippet(final String snippet) {
    return new TypeSafeMatcher<Object>() {
      ...
      public boolean matchesSafely(Object object) {
        ...
        ConversationListItem itemView    = (ConversationListItem) object;
        TextView             snippetView = (TextView) itemView.findViewById(com.tingtingapps.securesms.R.id.subject);
  ...
  public void testSaveTextDraft() throws Exception {
    ...
    final String DRAFT_SNIPPET  = getContext().getString(com.tingtingapps.securesms.R.string.ThreadRecord_draft) + " " + DRAFT_MESSAGE;
    ...
    onView(withId(com.tingtingapps.securesms.R.id.list)).check(matches(
