...
import com.tingtingapps.securesms.RegistrationProgressActivity;
...
public class RegistrationActivityTest extends TextSecureEspressoTestCase<ConversationListActivity> {
  ...
  private void loadStateAndCheck() throws Exception {
    ...
    onView(ViewMatchers.withId(com.tingtingapps.securesms.R.id.skipButton)).check(matches(not(isDisplayed())));
    onView(withId(com.tingtingapps.securesms.R.id.registerButton)).check(matches(isDisplayed()));
  }
